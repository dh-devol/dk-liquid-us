window.initMixMatchBundle = function(root) {
    ...
    function createGroupId(selections, addon) {
        if (bundleProductTitle.includes("Paint") || bundleProductTitle.includes("Candle")) {
    ...
    async function addToCart() {
        ...
        if (bundlePriceType == "variable") {
            ...
            const props = {
              ...
              _bundle_summary: compressed.map(x => x.qty > 1 ? `${x.title} (×${x.qty})` : x.title).join(' • '),
              __cartBundleId: FAST_BUNDLE_ID || undefined,
              _group_id: group_id || undefined,
              _bundle_sort: "1",
              _cart_text: product_cart_text || undefined
            };
            ...
            if (x.qty === 1) {
                props[`_${PREFIX} ${i + 1}`] = `${x.title}`;
            } else {
                props[`_${PREFIX} ${i + 1}`] = `${x.title} (×${x.qty})`;
            }
          });

            // NOTE: the Cart Transform Function reads `_bundle_items` and expands this
            // line into the chosen component line items at checkout, so the colours
            // already render as nested children. We therefore do NOT add visible
            // "Colour N" properties here — doing so duplicated the colours at checkout
        ...
        try {
            ...
            // Write lead time as an order attribute, keyed by the bundle product's ID
            if (bundlePriceType == "fixed" && bundleLeadTime && bundleLeadTime !== 'undefined' && bundleLeadTime !== '') {
                ...
                body: JSON.stringify({ attributes: leadAttrs })
                }).catch(() => {});
            }
