  ...
  customElements.define(
    ...
    class QuickOrderList extends BulkAdd {
      ...
      validateInput(target) {
        ...
        const step = parseInt(target.step);
        const normalizedStep = Number.isFinite(step) && step > 0 ? step : 1;

        if (target.max) {
          return (
            value == 0 ||
            (value >= min && value <= max && value % normalizedStep == 0)
          );
        }
        return value == 0 || (value >= min && value % normalizedStep == 0);
      }
      ...
      updateError(updatedValue, id, attemptedValue = null) {
        let message = '';
        if (typeof updatedValue === 'undefined') {
          message = window.cartStrings.error;
        } else {
          message = window.cartStrings.quantityError
            .replace('[attempted]', attemptedValue ?? updatedValue)
